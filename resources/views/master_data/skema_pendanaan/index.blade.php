...
@section('content')
  <section class="hero">
    <div class="hero-inner">
      <div class="hero-left">
        <h1 class="title">Skema Pendanaan</h1>
        <p class="sub">Kelola konfigurasi Skema Pendanaan beserta komponen biayanya.</p>
      </div>
      <div class="tools-row">
        <a href="{{ route('skema-pendanaan.create') }}" class="btn btn-brand"><i class="bi bi-plus-lg"></i> Tambah Skema</a>
      </div>
    </div>
  </section>
      ...
      <table id="table" class="table table-modern table-striped align-middle">
        <thead>
          <tr>
            ...
            <th class="text-start" width="25%">Nama Skema</th>
            <th class="text-start" width="20%">Hierarki</th>
            <th class="text-center" width="10%">Status</th>
            <th class="text-center" width="15%">Aksi & Komponen</th>
          </tr>
        </thead>
        <tbody>
          @foreach ($data as $r)
            <tr>
              ...
              <td class="text-center"><strong>{{ $r->kode }}</strong></td>
              <td class="text-start fw-bold">{{ $r->nama }}</td>
              <td class="text-start" style="font-size:0.8rem">
                <span class="badge bg-secondary">{{ $r->jenisProject->nama ?? '-' }}</span><br>
                <span class="badge bg-info mt-1">{{ $r->jenisPendanaan->nama ?? '-' }}</span><br>
                <span class="badge bg-primary mt-1">{{ $r->provider->singkatan ?? '-' }}</span>
              </td>
              ...
              <td class="text-center">
                <div class="d-flex justify-content-center gap-2">
                  <a href="{{ route('skema-pendanaan.edit', $r->id) }}" class="btn btn-warning btn-sm" title="Edit Skema & Komponen"><i class="bi bi-gear-fill"></i> Konfigurasi</a>
                  <form action="{{ route('skema-pendanaan.destroy', $r->id) }}" method="POST" class="d-inline" data-id="{{ $r->id }}">
                    @csrf @method('DELETE')
...
@push('scripts')
  ...
  <script>
    ...
    function confirmDelete(id){
      Swal.fire({
        title:'Hapus skema ini?', text:'Tindakan ini tidak dapat dibatalkan.', icon:'warning', showCancelButton:true, confirmButtonColor:'#d33', confirmButtonText:'Ya, hapus'
